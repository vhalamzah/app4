...
@section('content')
    ...
    <p> <h4>Are you sure you want to delete <strong> {{$permission->name}} ?</strong></h4></p>

                    <form method="POST" action="{{ route('permissions.destroy', ['id' => $permission->id]) }}">
                        <input type="hidden" name="_token" value="{{ Session::token() }}">
                        <input name="_method" type="hidden" value="DELETE">
                        <a  href="{{route('permissions.show', $permission->id)}}"  class="btn btn-primary">No do not Delete</a>
